...
public class StorageStoredItem : MonoBehaviour
{
    ...
    void Update()
    {
        itemHolder.localScale = Vector3.Lerp(itemHolder.localScale, Vector3.one * (mouseOver ? 1.1f : 0.95f), 10 * Time.deltaTime);
        ...
        {
            storageChestManager.TakeItem(storageSlotId);
        }
        if (Input.GetMouseButtonDown(1) && mouseOver && thisItemId > -1)
        {
            storageChestManager.RemoveSItem(storageSlotId);
        }
    }
    ...
    {
        if (thisItemId > -1)
            Instantiate(endPrefab, transform.position, Quaternion.identity).GetComponent<StorageItemEnd>().UpdateItem(ItemSpriteDictionary.itemSprites[thisItemId], thisItemRarityColor, 0.65f);
    }
    public void SpawnRemObj()
    {
        if (thisItemId > -1)
            Instantiate(endPrefab, transform.position, Quaternion.identity).GetComponent<StorageItemEnd>().UpdateItem(ItemSpriteDictionary.itemSprites[thisItemId], thisItemRarityColor, 0.65f, true);
    }
